...
def e_step_wrapper(q):
    ...
    e_step.coordinate_ascent_training(prediction=q[10])
...
if __name__ == '__main__':
    ...
    query_list = [(fpath, K, {j:input_data_x[j] for j in range(i,i+m)}, list_to_dict(input_data_y, i, i+m), new_alpha, new_eta, new_delta, new_Lambda, epsilon, predict) for i in range(0,len(input_data_y)-m,m)]
    r = len(input_data_y) % m
    if r == 0:
        i = len(input_data_y) - m
    else:
        i = len(input_data_y) - r
    query_list.append((fpath, K, {j:input_data_x[j] for j in range(i,len(input_data_y))}, list_to_dict(input_data_y,i,len(input_data_y)), new_alpha, new_eta, new_delta, new_Lambda, epsilon, predict))
